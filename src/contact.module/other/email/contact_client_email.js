...
const { translator } = require('./contact_client_email_translator');
...
async function sendEmail(dataContact, dataCommentary){
    try {
            ...
            <div style="width: 100%; display: flex; justify-content: center; flex-wrap: wrap;">
                <div style="width: 100%; display: flex; justify-content: center; flex-wrap: wrap; background: linear-gradient(150deg, rgb(138,43,226), rgba(138,43,226,0.5));">
                    ...
                    <div style="width: 100%; text-align: center; padding: 10px;">
                        <a href="http://www.brixpi.com/register">
                            <button style="width: auto; height: auto; padding: 10px 15px; border: none; background-color: rgba(250,5,217,1); 
                                color: white; border-radius: 15px; font-family: Arial, Helvetica, sans-serif; font-size: 12px; font-weight: bold; cursor: pointer;">
                                ${translator(siteLanguage).register}
                            </button>
                        </a>
                    </div>
                </div>
                <div style="width: 100%; background-color: white; display: flex; justify-content: center; flex-wrap: wrap;">
                    <div style="width: 100%; height: auto; padding: 0 10px;">
                        <p style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; color: rgba(0,0,0,1); text-align: left;">${translator(siteLanguage).hello} ${dataContact.name}, ${translator(siteLanguage).hello2}</p>
                    </div>
                    <div style="width: 100%; height: auto; padding: 0 10px;">
                    ...
                    <div style="width: 70%; background-color: white; border-top: solid 1px rgba(238, 232, 241,1); padding: 10px;">
                        <p style="font-family: Arial, Helvetica, sans-serif; font-size: 12px; color: rgba(0,0,0,0.6);">
                            ${translator(siteLanguage).info1} <a href="http://www.brixpi.com/register">${translator(siteLanguage).info2}.</a>
                            ${translator(siteLanguage).info3}
                        </p>
                    </div>
                    <div style="border-top: solid 1px rgba(238, 232, 241,1); width: 70%; padding: 10px;">
                        <span style="font-family: Arial, Helvetica, sans-serif; font-size: 12px; color: rgba(0,0,0,0.6);">2022 brixpi</span>
                    </div>
                </div>
            </div>
        `;
        ...
        const result = await transporter.sendMail({
            ...
            subject: `${translator(siteLanguage).subject1} ${dataContact.name}, ${translator(siteLanguage).subject2}. `,
            //...text: 'probando datos'...
            html: contentHtml
        });

        if (result === null) return {code: 'CONT0020'};

        return {code: 'CONT0021'};
    }
    catch (error) {
        return {code: 'CONT0019'};
    }
}
