...
const { translator }  = require('./user_register_email_translator');
...
const sendEmail = async (data) => {
    try {
        ...
        /* let contentHtml = `
            ...
            <p>Tu registración ha comenzado correctamente!. Para continuar y finalizar, copia el siguiente código 
            y pégalo en el casillero correspondiente.
            </p>
            <p>CÓDIGO:</p>
            <p>${data.codeSendParam}</p>
            <p>Si tienes dudas envíanos un email y nos contactaremos a la brevedad. Muchas gracias!</p>
        `; */
        ...
        if (result === null) return {code: 'USER0027'};

        return {code: 'USER0028'};
    } 
    catch (error) {
        return {code: 'USER0026'};
    }
}
